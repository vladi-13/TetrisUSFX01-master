...
ABoard::ABoard()
{
    ...
    //Create the boards
    AEscenarios* Escena = GetWorld() -> SpawnActor<ARealEscenario>(ARealEscenario::StaticClass());
}
...
void ABoard::BeginPlay()
{
    ...
    for (int i = 0; i <= 1; i++)
    {
        ASinglePlayer* SpawnedSinglePlayer = GetWorld() -> SpawnActor<ASinglePlayer>(ASinglePlayer::StaticClass());
        if (SpawnedSinglePlayer)
        {
            //If the Spawn succeeds, set the Spawned inventory to the local one
            //and log the success string
            SinglePlayer = SpawnedSinglePlayer;
            GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow,FString::Printf(TEXT("%s has been created"), *SinglePlayer->GetName()));
        }
    }

    //Spawn Builder and Engineer
    PiezasBuilder = GetWorld()->SpawnActor<APiezaRandomicaBuilder>(APiezaRandomicaBuilder::StaticClass());
    Ing = GetWorld()->SpawnActor<APiezaIng>(APiezaIng::StaticClass());
    //Set the Builder for the Engineer and create the buildings
    Ing->setPiezaBuilder(PiezasBuilder);
    Ing->constructPieza();
    //Get the Engineer's Lodging and Logs the created buildings
    APiece* Piece = Ing->getPiece();
    //Piece->LodgingCharacteristics();
